...
const BudgetPage: React.FC<ToolPageProps> = ({ isFullscreen, setIsFullscreen }) => {
  ...
  const handleDownloadBudgetPDF = () => {
    if (expenses.length === 0 && budgetTotal === 0) return;
  ...
  const TabButton: React.FC<{tabId: TabId, activeTab: TabId, setActiveTab: (id: TabId) => void, children: React.ReactNode}> = ({ tabId, activeTab, setActiveTab, children }) => (
    <button
